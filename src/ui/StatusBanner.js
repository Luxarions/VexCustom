...
export class StatusBanner {
  ...
  render() {
    ...
    banner.innerHTML = `
      ...
      <div class="banner-actions">
        ...
        <button id="btn-clear-term" class="btn btn-secondary">Clear Console</button>
      </div>
    `;
  ...
  setRunning() {
    ...
    if (this.summaryEl) {
      this.summaryEl.textContent = 'Executing 9 test suites sequentially in browser runtime...';
    }
    if (this.runBtn) {
   ...
   */
  setCompleted(passed, failed, duration) {
    if (this.runBtn) {
      this.runBtn.disabled = false;
    }
    if (this.btnText) {
      this.btnText.textContent = 'Rerun All Tests';
    }
    if (failed === 0) {
      if (this.badgeEl) {
      ...
      if (this.summaryEl) {
        this.summaryEl.textContent = `All 9 cryptographic, codec, and boundary test suites passed cleanly in ${duration.toFixed(1)}ms.`;
      }
    } else {
      if (this.badgeEl) {
